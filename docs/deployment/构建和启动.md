...
`uv sync` 后无需打包，可直接用 venv 内的 Python 启动：

```bash
# 加载环境变量
source ./build_test/registry.env

# 方式一：控制台脚本（pip install . 后可用）
a2x-registry

# 方式二：python -m（开发推荐，无需 install）
python -m a2x_registry.backend

# 方式三：带 --port 覆盖（env 优先级更高）
python -m a2x_registry.backend --port 9000
```
