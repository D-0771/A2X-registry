"""schema 校验：预制 .db 文件的表 / 列 / 索引必须与实现文档 §3.2 一致。

锁定表结构的真源，防止 P0-2 `register/store.py` 实现时漂移。
所有断言对照 `一体机Agent OS registry实现文档.md` §3.2。
"""
...
def test_init_schema_is_idempotent(fresh_conn):
    ...
    对应 P0-1 要求："启动时一次性建齐（CREATE TABLE IF NOT EXISTS）"。
